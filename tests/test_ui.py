...
class DiscoveryUiTests(unittest.TestCase):
    ...
    def test_camera_lifecycle_actions_are_grouped_without_hiding_primary_actions(self) -> None:
        ...
        self.assertIn('addText(actionStack, "button", "row-action", "More")', self.html)
        self.assertIn('openAppModal("actions"', self.html)
        self.assertIn('addText(sheet, "button", "danger-action", "Unadopt")', self.html)
        self.assertIn('headers: {"X-CamAdmiral-Action": "unadopt-camera"}', self.html)
        self.assertIn('addText(sheet, "button", "danger-action", "Block device")', self.html)
