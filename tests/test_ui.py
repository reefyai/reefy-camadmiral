...
class DiscoveryUiTests(unittest.TestCase):
    ...
    def test_dashboard_scan_action_owns_status_and_diagnostics_modal(self) -> None:
        ...
        self.assertIn('id="scan-network-restore"', self.html)
        ...
        self.assertIn('resetScanModalPresentation()', self.html)
        self.assertIn('scanModalScanId = activeState?.scan_id || null', self.html)
        ...
        self.assertIn('restorePrompt.textContent = "Restore detected subnet..."', self.html)
        self.assertIn('scanNetworkRestore.addEventListener("change", async () =>', self.html)
