...
def assert_camera_unadopt_block_and_restore(page: Page) -> None:
    ...
    onvif_row.get_by_role("button", name="More").click()
    expect(page.locator("#app-modal-title")).to_contain_text("actions")
    page.locator("#app-modal").get_by_role("button", name="Unadopt", exact=True).click()
    ...
    onvif_row.get_by_role("button", name="More").click()
    page.locator("#app-modal").get_by_role("button", name="Block device").click()
    expect(page.locator("#confirm-modal")).to_be_visible()
    with page.expect_response(
        lambda response: response.request.method == "POST"
