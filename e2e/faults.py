...
def identity_consumer() -> None:
    ...
    for stream_key, stream in streams().items():
        ...
        for consumer in stream.get("consumers") or []:
            ...
            if user_agent.startswith(IDENTITY_CONSUMER_PREFIX):
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"Expected one identity E2E consumer, found {len(matches)}: {matches}"
        )
...
def assert_onvif_runtime_config_moved() -> None:
    ...
    if moved_paths != {"/main", "/sub"} or stale:
        raise RuntimeError(
            "go2rtc runtime config did not persist both moved ONVIF sources"
        )
    print("go2rtc runtime config contains only the moved ONVIF sources")
