...
def replace_streams(sources: list[dict[str, str]]) -> None:
    ...
    for source in sources:
        uri = authenticated_rtsp_uri(source["uri"], source["username"], source["password"])
        # PATCH preserves go2rtc's current Stream object and its attached consumers.
        _request("PATCH", "/api/streams", {"name": source["stream_key"], "src": uri})
        persisted[source["stream_key"]] = [uri]
    if persisted:
        # The streams PATCH is runtime-only. Persist the same DB-derived sources
        # so a supervised go2rtc child restart does not restore stale addresses.
        body = json.dumps(
            {"streams": persisted},
            sort_keys=True,
            separators=(",", ":"),
